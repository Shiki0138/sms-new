...
class SettingsManager {
    ...
    async loadChannelConfigs() {
        try {
            ...
            console.log('Channel config API response:', data);
            
            // Handle the actual API response format
            const channels = data.success ? data.data.channels : (data.configs || data.channels || []);
            this.channelConfigs = channels.reduce((acc, config) => {
                acc[config.id || config.channel] = config;
                return acc;
            }, {});
