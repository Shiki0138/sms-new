...
const ReservationsPageAdvanced: React.FC = () => {
  ...
  const displayDates = useMemo(() => {
    switch (viewType) {
      ...
      case 'month': {
        ...
        const weeksCount =
          Math.ceil(
            (monthEnd.getTime() - calendarStart.getTime()) /
              (7 * 24 * 60 * 60 * 1000)
          ) + 1;
  ...
  const configuredHolidays = useMemo(() => {
    ...
    if (businessHoursError) {
      console.error(
        'ReservationsPage - Error loading business hours:',
        businessHoursError
      );
      return mockHolidays;
    }
    ...
    console.log(
      `ReservationsPage - Got ${holidays.length} holidays from settings`
    );
    ...
    if (holidays.length > 0) {
      ...
      console.log(
        'Current month Mon/Tue holidays:',
        currentMonthHolidays.map((h) => format(h, 'yyyy-MM-dd (E)'))
      );
    }
  ...
  const isHoliday = (date: Date) => {
    ...
    if (dayOfWeek === 1 || dayOfWeek === 2) {
      ...
      console.log(
        `Holiday check for ${dateString} (${dayOfWeek === 1 ? 'Monday' : 'Tuesday'}): ${isHolidayDate ? '✅ Holiday' : '❌ Not holiday'}`
      );
    }
  ...
  const isBusinessTime = (date: Date, time?: Date) => {
    ...
    const isWithinHours =
      timeStr >= businessHour.openTime && timeStr <= businessHour.closeTime;
  ...
  const getReservationsForDateTime = (date: Date, time?: Date) => {
    return mockReservations.filter((reservation) => {
      ...
      if (time) {
        ...
        return (
          (reservation.startTime >= slotStart &&
  ...
  const renderReservationBlock = (
    ...
    return (
      <div
        ...
        style={{
          borderLeftColor: reservation.staffColor,
          borderLeftWidth: '4px',
        }}
        onClick={() =>
          alert(
            `予約詳細: ${reservation.customerName} - ${reservation.serviceName}`
          )
        }
      >
              ...
              <div key={date.toISOString()} className="flex-1 min-w-0">
                ...
                <div className="relative">
                  {timeSlots.map((time) => {
                    ...
                    const slotReservations = getReservationsForDateTime(
                      date,
                      time
                    );
                    ...
                    return (
                      <div
                        ...
                        onClick={() => {
                          if (isBusinessSlot) {
                            alert(
                              `${format(date, 'M月d日')} ${format(time, 'HH:mm')}の予約作成（デモ版）`
                            );
                          }
                        }}
                      >
            ...
            <div className="flex items-center space-x-4 text-xs text-gray-500">
              ...
              {!businessHoursLoading && (
                <div className="flex items-center space-x-1 text-blue-600">
                  ...
                  <span>
                    設定から読み込み済み（{configuredHolidays.length}件の休日）
                  </span>
                </div>
              )}
            </div>
      ...
      <Card className="mt-6">
        <div className="p-4">
          ...
          <div className="space-y-3">
            {mockReservations
              .filter((reservation) =>
                isSameDay(reservation.startTime, new Date())
              )
              .map((reservation) => (
                <div
                  key={reservation.id}
