...
/**
 * 営業時間・休日管理サービス (Fixed version with proper tenant ID handling)
 */
export class BusinessHoursService {
  ...
  /**
   * Ensure tenant ID is in proper format
   */
  private async getValidTenantId(): Promise<string> {
    // First check if the tenant ID is already a valid UUID
    const uuidRegex =
    ...
    // If not, try to get the actual tenant ID from the database
    try {
      ...
      if (!data?.tenant_id) throw new Error('No tenant ID found for user');

      return data.tenant_id;
    } catch (error) {
      console.error('Error getting valid tenant ID:', error);
      // Return the original tenant ID and let the database handle the error
      return this.tenantId;
    }
  }
  ...
  async getBusinessHours(): Promise<BusinessHour[]> {
    try {
      const validTenantId = await this.getValidTenantId();
  ...
  async getHolidaySettings(): Promise<HolidaySetting[]> {
    try {
      ...
      console.log('[getHolidaySettings] Fetching for tenant:', validTenantId);
      ...
      if (error) {
        ...
        if (error.code === '42P01') {
          console.error(
            '[getHolidaySettings] Table holiday_settings does not exist'
          );
          return [];
        }
  ...
  async createHolidaySetting(holidayData: CreateHolidayData): Promise<{
    ...
    try {
      ...
      console.log('[createHolidaySetting] Valid Tenant ID:', validTenantId);
      ...
      if (error) {
        ...
        if (error.code === '42P01') {
          ...
          return {
            ...
            success: false,
            error:
              'テナント情報が正しくありません。ページを更新してもう一度お試しください。',
          };
        }
